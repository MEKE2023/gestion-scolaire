...
import {
  ...
  Check, X, ArrowUpDown, GraduationCap, Menu as MenuIcon, Award,
} from "lucide-react";
...
const initStudents = () => ([
  { id: "e1", prenoms: "Léa", nom: "Bernard", sexe: "F", naissance: "2021-03-12", matricule: "MAT-0001", classeId: "cl1", parent: "Mme Bernard", telephone: "07 01 02 03 04", photo: null },
  { id: "e2", prenoms: "Noah", nom: "Petit", sexe: "M", naissance: "2021-06-02", matricule: "MAT-0002", classeId: "cl1", parent: "M. Petit", telephone: "07 02 03 04 05", photo: null },
  { id: "e3", prenoms: "Inès", nom: "Dubois", sexe: "F", naissance: "2020-01-20", matricule: "MAT-0003", classeId: "cl2", parent: "Mme Dubois", telephone: "07 03 04 05 06", photo: null },
  { id: "e4", prenoms: "Rayan", nom: "Fontaine", sexe: "M", naissance: "2020-08-11", matricule: "MAT-0004", classeId: "cl2", parent: "M. Fontaine", telephone: "07 04 05 06 07", photo: null },
  { id: "e5", prenoms: "Camille", nom: "Roy", sexe: "F", naissance: "2019-04-09", matricule: "MAT-0005", classeId: "cl3", parent: "Mme Roy", telephone: "07 05 06 07 08", photo: null },
  { id: "e6", prenoms: "Hugo", nom: "Simon", sexe: "M", naissance: "2019-11-30", matricule: "MAT-0006", classeId: "cl3", parent: "M. Simon", telephone: "07 06 07 08 09", photo: null },
]);
const initTranches = () => ([
...
function Card({ children, style }) {
  return <div style={{ background: C.paperCard, border: `1px solid ${C.line}`, borderRadius: 12, padding: 16, marginBottom: 14, ...style }}>{children}</div>;
}
...
function Btn({ children, onClick, kind = "primary", style, type = "button" }) {
  const styles = {
...
export default function App() {
  ...
  const [config, setConfig] = useState({ devise: "GNF", etablissement: "Mon École", bareme: 20, comptaPassword: "compta2026" });
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (data && data.data) {
        ...
        setConfig(d.config || { devise: "GNF", etablissement: "Mon École", bareme: 20, comptaPassword: "compta2026" });
        ...
        await supabase.from("app_state").insert({
          ...
          data: {
            ...
            notes: initNotes(), config: { devise: "GNF", etablissement: "Mon École", bareme: 20, comptaPassword: "compta2026" },
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      supabase.from("app_state").upsert({
        id: "main",
        data: { classes, students, tranches, paiements, staff, paieHist, depenses, matieresConfig, notes, config, periodes },
        updated_at: new Date().toISOString(),
      }).then();
    }, 800);
    return () => clearTimeout(t);
  }, [classes, students, tranches, paiements, staff, paieHist, depenses, matieresConfig, notes, config, periodes, session, dataLoaded]);
      ...
      <div>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14, flexWrap: "wrap", gap: 8 }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            ...
            <Btn onClick={() => setEleveForm({ prenoms: "", nom: "", sexe: "F", naissance: "", matricule: "", classeId: classes[0]?.id || "", parent: "", telephone: "", photo: null })}><Plus size={13} /> Ajouter un élève</Btn>
          </div>
        </div>
  ...
  const renderBulletin = () => {
    ...
    const noteOf = (matiereId) => notes.find(n => n.studentId === bulEleve && n.matiereId === matiereId && n.trimestre === bulTrimestre)?.note ?? null;
    ...
    return (
      <div>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 14, flexWrap: "wrap" }}>
          ...
          <Select value={bulEleve} onChange={e => setBulEleve(e.target.value)}><option value="">Choisir un élève…</option>{classeEleves.map(s => <option key={s.id} value={s.id}>{s.prenoms} {s.nom}</option>)}</Select>
        </div>
        ...
        <Card>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 12 }}>
            <div>
              <div style={{ fontSize: 11, color: C.textSoft, marginBottom: 4 }}>Nom de l'établissement (en-tête du bulletin)</div>
        ...
        {bulEleve ? (
          <Card className="print-area">
            <div className="f-display" style={{ textAlign: "center", fontWeight: 600, fontSize: 15, color: C.text, marginBottom: 10 }}>{config.etablissement}</div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
              <div>
                <div style={{ fontWeight: 700 }}>{students.find(s => s.id === bulEleve)?.prenoms} {students.find(s => s.id === bulEleve)?.nom}</div>
                <div style={{ fontSize: 11, color: C.textSoft }}>{classes.find(c => c.id === bulClasse)?.nom} — {bulTrimestre}</div>
              </div>
              <Btn kind="ghost" className="no-print" onClick={() => window.print()}><Printer size={13} /> Imprimer</Btn>
            </div>
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              <thead><tr><Th>Matière</Th><Th>Coef</Th><Th>Note</Th><Th>Note coef.</Th><Th>Rang</Th><Th>Mention</Th></tr></thead>
              <tbody>
                {matieres.map(m => {
                  const n = noteOf(m.id);
                  return (
                    <tr key={m.id}>
                      ...
                      <Td className="f-mono">{m.coef}</Td>
                      <Td>
                        <input type="number" min="0" max={config.bareme} step="0.5" value={n ?? ""} placeholder="—"
                          onChange={e => setNote(bulEleve, m.id, bulTrimestre, e.target.value)}
                          style={{ width: 56, padding: "4px 6px", borderRadius: 6, border: `1px solid ${C.line}`, fontSize: 12.5 }} />
                      </Td>
                      <Td className="f-mono">{n != null ? (n * m.coef).toFixed(1) : "—"}</Td>
                      <Td>{rangMatiere(m.id, bulTrimestre, bulEleve)}</Td>
                      <Td><Pill_ text={mention(n)} color={n >= 12 ? C.sage : n >= 10 ? C.brass : C.rose} bg={n >= 12 ? C.sageSoft : n >= 10 ? C.brassSoft : C.roseSoft} /></Td>
                    </tr>
                  );
                })}
              </tbody>
            </table>
            <div style={{ marginTop: 12, paddingTop: 10, borderTop: `1px solid ${C.line}`, display: "flex", justifyContent: "space-between", fontWeight: 700 }}>
              <span>Moyenne générale pondérée</span>
              <span className="f-mono">{moyenne ?? "—"} / {config.bareme} — {mention(moyenne ? Number(moyenne) : null)}</span>
            </div>
          </Card>
        ) : <Card><div style={{ fontSize: 12, color: C.textSoft }}>Sélectionnez un élève pour saisir ses notes et afficher le bulletin.</div></Card>}
      </div>
    );
  };
  ...
  const renderComptabilite = () => {
    ...
    const subtabs = [
      ["effectifs", "Statistiques"], ["statut", "Statut par élève"], ["tranches", "Tranches"], ["paiement", "Paiement"],
          ...
          <Card style={{ padding: 0, overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              ...
              <tbody>{students.map(s => { const paid = studentPaid(s.id); const reste = totalTranches - paid;
                return (
                  <tr key={s.id}>
                    <Td style={{ fontWeight: 600 }}>{s.prenoms} {s.nom}</Td><Td>{classes.find(c => c.id === s.classeId)?.nom}</Td>
        ...
        {compTab === "paiement" && (
          <Card>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 10 }}>
              <Select value={paieForm.studentId} onChange={e => setPaieForm({ ...paieForm, studentId: e.target.value })}><option value="">Élève…</option>{students.map(s => <option key={s.id} value={s.id}>{s.prenoms} {s.nom}</option>)}</Select>
            ...
            {recu && (
              <div className="print-area" style={{ marginTop: 16, border: `1px dashed ${C.brass}`, borderRadius: 10, padding: 16 }}>
                <div style={{ textAlign: "center", fontWeight: 700, marginBottom: 8 }}>REÇU DE PAIEMENT — N° {recu.id}</div>
                <div style={{ fontSize: 13, lineHeight: 1.8 }}>
                  Élève : <b>{students.find(s => s.id === recu.studentId)?.prenoms} {students.find(s => s.id === recu.studentId)?.nom}</b><br />
                  Tranche : {tranches.find(t => t.id === recu.trancheId)?.nom}<br />
                  Montant : <b>{fmt(recu.montant)}</b><br />
                  Mode : {recu.mode} — Date : {recu.date}
                </div>
                <Btn kind="ghost" className="no-print" onClick={() => window.print()} style={{ marginTop: 10 }}><Printer size={13} /> Imprimer le reçu</Btn>
              </div>
            )}
          </Card>
        )}
          ...
          <Card style={{ padding: 0, overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              ...
              <tbody>{[...paiements].sort((a, b) => b.date.localeCompare(a.date)).map(p => (
                <tr key={p.id}><Td>{p.date}</Td><Td>{students.find(s => s.id === p.studentId)?.prenoms} {students.find(s => s.id === p.studentId)?.nom}</Td><Td>{tranches.find(t => t.id === p.trancheId)?.nom}</Td><Td className="f-mono">{fmt(p.montant)}</Td><Td>{p.mode}</Td></tr>
              ))}</tbody>
            </table>
          </Card>
        )}
          ...
          <div>{alertes.length === 0 ? <Card>Aucune alerte : tous les paiements en retard sont à jour.</Card> :
            alertes.map((a, i) => (
              <Card key={i} style={{ display: "flex", alignItems: "center", gap: 10, borderColor: C.rose }}>
                ...
                <div><b>{a.student.prenoms} {a.student.nom}</b> — {a.tranche.nom} en retard <span style={{ color: C.textSoft, fontSize: 11 }}>(échéance {a.tranche.limite})</span></div>
              </Card>
            ))}
          </div>
        )}
  ...
  const pages = { accueil: renderAccueil, eleves: renderEleves, classes: renderClasses, bulletin: renderBulletin, comptabilite: renderComptabilite };
